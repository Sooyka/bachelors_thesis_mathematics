...
\todo{Stationary distribution, eigenbasises}
%\textbf{\Huge BAM!}\normalsize
...
is made in $a$-decks, so standard shuffle is a $2$-shuffle. In the forward case we are spliting the deck
...
together. More specific analisis of $a$ shuffels can be done with Hopf algebras.
%/////////////////////////////////////////////////////////////////////////////////////////////////////////
%/////////////////////////////////////////////////////////////////////////////////////////////////////////
%/////////////////////////////////////////////////////////////////////////////////////////////////////////
%/////////////////////////////////////////////////////////////////////////////////////////////////////////
...
\end{observation}
\begin{proof}
The proof is immediate from the \hyperref[observation:1]{Observation 1.}. Since if
$\{b_i\}_{i \in I}$, $\{c_j\}_{j \in J}$ are bases of, respectively, $V$and $W$ and
...
\end{observation}
\begin{proof}
This observation is just a recollection of the definition.
\end{proof}
\begin{observation}
...
$U \otimes V \otimes W$ or a product of any greater number of vector spaces in that way. (We will also
write "$u \otimes v \otimes w$" for some of their elements.) Form of elements, operations on them and
structure of that vector spaces are fully analogous to the described above (with respect to all
...
\text{$m:\mathcal{H} \otimes \mathcal{H} \to \mathcal{H}$} called multiplication and a linear map
...
no matter which set of parts ($a_1$ or $a_2$) was been taken in the second iteration. It can become more
...
\begin{gather*}
    ...
    {^Km}(\varepsilon \otimes \varepsilon)(X^n \otimes X^m) = {^Km}(\varepsilon(X^n)\otimes 0) = 0
\end{gather*}
and for $n = 0$, $m = 0$
\begin{gather*}
    \varepsilon m(X^0 \otimes X^0) = \varepsilon (1_P \otimes 1_P) = 1_K \mathrm{\ and}\\
    {^Km}(\varepsilon \otimes \varepsilon)(X^0 \otimes X^0) =
    {^Km}(\varepsilon \otimes \varepsilon)(1_P \otimes 1_P) = {^Km}(1_K \otimes 1_K) = 1_K,
\end{gather*}
so $\varepsilon m = ^Km(\varepsilon \otimes \varepsilon)$.
\item for all $k \in K$
\begin{gather*}
    \Delta u(k) = \Delta(k1_P) =
    \Delta(kX^0) = k\Delta(X_0) = k(X^0 \otimes X^0) \mathrm{\ and} \\
    (u\otimes u){^K\Delta}(k) = (u\otimes u) (1_K \otimes k) = k(u \otimes u)(1_K \otimes 1_K) =
...
\begin{gather*}
    \Delta(X^n) = \sum_{i=0}^n X^i \otimes X^{n - i} = \sum_{i=0} X^{n-i} \otimes X^i
\end{gather*}
...
In the simillar way for $s \in \mathcal{X}^*$:
\begin{equation}
    \Psi^{[a]}(s) = m^{[a]}\Delta^{[a]}(s) =
    \sum_{\substack{s_1, \dots, s_a\prec s\\ \mathrm{pairwise\ disjoint}}}s_1\dots s_a
\end{equation}
%/////////////////////////////////////////////////////////////////////////////////////////////////////////
%/////////////////////////////////////////////////////////////////////////////////////////////////////////
%/////////////////////////////////////////////////////////////////////////////////////////////////////////
...
(a^* \otimes b^*)\left(\sum_{\substack{s_1 \prec s \\ s_2 = s / s_1}} s_1 \otimes s_2 \right)=\\
\sum_{\substack{s_1 \prec s \\ s_2 = s / s_1}} a^*(s_1) \otimes b^*(s_2)
\end{gather*}
which is equal to one on those $s$ for which holds $a \prec s$ and $b = s/a$, and zero on all other elements
of $s$, so indeed
\[
\Delta^*(a^* \otimes b^*) =
\sum\{s^* : a \prec s \mathrm{\ and\ } b = s/a\}.
\]
So we have:
\[
\Delta^*(\varphi[a]\otimes \varphi[b]) = \Delta^*(a^* \otimes b^*) =
\sum\{s^* : a \prec s \mathrm{\ and\ } b = s/a\} = \varphi\left[\Delta^*(a \otimes b)\right].
\]
As well:
\begin{equation*}
...
\]
That gives us, that for all $\nu \in \mathcal{X}^*$, for all $a \in \mathbb{N}$ transformations $\Psi$
and $\Psi^*$ on $\mathcal{H}_\nu$ are conjugate (and that is the
reason why we are denoting them with "$^*$").
\todo{matrixes?}
%//////////////////////////////////////////////////////////////////////////////////////////////////////////
%/////////////////////////////////////////////////////////////////////////////////////////////////////////
%/////////////////////////////////////////////////////////////////////////////////////////////////////////
...
Analisis of $\mathcal{H}$ and $\mathcal{H}^*$ (see \ref{gcha}) gives that accessible states are exactly
the same as in, accordingly inverse and forward riffle shuffle. So generated Markov chains are exactly
...
\\
As for all $\nu$ transformation $\Psi$ and $\Psi^*$ on $\mathcal{H}_\nu$ are conjugated to each other
it gives us, that their matrices are transpositions of each other, so matrices of corresponding chains are
transpositions of each other, so indeed forward and backward riffle shuffling is one method of shuffling
once applied forward and once backward in the sense that (for $(F_0, F_1, \dots)$ for forward and
$(I_0, I_1, \dots)$ for
inverse rifffle shuffling):
\begin{equation*}
\mathbb{P}(F_{n+1} = s_2 \mid F_n = s_1) = \mathbb{P}(I_{n+1} = s_1 \mid I_n = s_2).
\end{equation*}
\todo{theorem of the above, not so powerfull}
