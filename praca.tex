...
%/////////////////////////////////////////////////////////////////////////////////////////////////////////
\chapter{Connection}
\todo{to jest źle, ale będzie fajniej}
\begin{lemma}
Let $V$ be an linear space over $\mathbb{R}$ or $\mathbb{Q}$ with basis $\mathcal{B}$.
Let $\psi : V \to V$ be a linear operation. If for
all $b \in \mathcal{B}$ the sum $s$ of coefficients of $\psi(b)$ written in $\mathcal{B}$ is the same, then
$\psi$ sets the Markov chain $(X_0, X_1, \dots)$ on $\mathcal{B}$ by for each $n \geq 1$, each
$b \in \mathcal{B}$
defining $\mathbb{P}\{X_n = b_i\}$
as $ $ with probability of going from $b_1$ to $b_2$ equal to the
